...
class VGG11KernelExperiment(nn.Module):
    def __init__(self, num_classes=10, conv_configs=None):
        super(VGG11KernelExperiment, self).__init__()
        if conv_configs is None:
            # kernel_size: 3x3,5x5, 7x7
            conv_configs = [
                (3, 64, 5, 1, 2),  # Block 1, kernel_size=5
                (64, 128, 5, 1, 2),  # Block 2, kernel_size=5
                (128, 256, 3, 1, 1), (256, 256, 3, 1, 1),  # Block 3, kernel_size=3
                (256, 512, 7, 1, 3), (512, 512, 7, 1, 3),  # Block 4, kernel_size=7
                (512, 512, 2, 1, 0), (512, 512, 2, 1, 0),  # Block 5, kernel_size=2
            ]
...
def train_vgg11_kernel_experiment(model, train_loader, test_loader, device, epochs):
    """
    Train the experimental VGG11 model with kernel size variations.
    Args:
    ...
    for epoch in range(epochs):
        ...
        validate_vgg11_kernel_experiment(model, test_loader, device)

    return model


def validate_vgg11_kernel_experiment(model, test_loader, device):
    """
    Validate the experimental VGG11 model with kernel size variations.
    Args:
